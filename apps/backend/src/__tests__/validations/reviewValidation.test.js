...
describe('createReviewSchema', () => {
  it('validates valid review data', async () => {
    const result = await createReviewSchema.validate({
      globalRating: 4,
      content:
        'This is a great internship experience. The team was supportive and I learned a lot about development.',
    });
  ...
  it('rejects missing globalRating', async () => {
    await expect(
      createReviewSchema.validate({
        content: 'This is a great internship experience with lots of content to write here.',
      })
  ...
  it('rejects globalRating below 1', async () => {
    await expect(
      createReviewSchema.validate({
        globalRating: 0,
        content: 'This is a great internship experience with lots of content.',
      })
  ...
  it('rejects missing content', async () => {
    await expect(createReviewSchema.validate({ globalRating: 3 })).rejects.toThrow(
      'content is required'
    );
  });
  ...
  it('accepts all ratings from 1 to 5', async () => {
    const longContent =
      'This is a detailed review of the internship with enough content to pass the 50 character minimum.';
    for (let i = 1; i <= 5; i++) {
