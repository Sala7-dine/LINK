    ...
    <div className="min-h-screen flex overflow-hidden bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-between overflow-hidden bg-zinc-950 p-12">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {['Offres de stage', 'Suivi des candidatures', 'Réseau alumni', 'Profil étudiant'].map(
              (f) => (
                <span
                  key={f}
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-6 opacity-40">
            {['YouCode', 'Simplon', '1337', 'EMSI', 'OFPPT'].map((name) => (
              <span key={name} className="text-white font-bold text-sm">
                {name}
              </span>
            ))}
          </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="lg:hidden flex items-center justify-between p-6 border-b border-zinc-100">
          ...
          <Link
            to="/login"
            className="text-sm text-zinc-500 hover:text-green-600 transition-colors"
          >
            Connexion
          </Link>
        </div>
