...
export default function Sidebar() {
  ...
  if (isSchoolAdmin) {
    nav = [
      {
        to: user?.role === 'school_admin' ? '/admin/dashboard' : '/platform/dashboard',
        label: 'Statistiques',
        icon: HomeIcon,
      },
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-full bg-white/60 backdrop-blur-2xl border-r border-white/40 shadow-[4px_0_24px_-4px_rgba(0,0,0,0.05)] z-20 transition-all duration-300 ${open ? 'w-64' : 'w-20'}`}
    >
      <div className="flex items-center gap-3 h-20 px-5 border-b border-zinc-200/50 bg-white/30">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-green-500 to-emerald-600 shadow-md flex items-center justify-center text-white font-bold text-xl select-none ring-2 ring-white/50 flex-shrink-0">
          L
        </div>
        ...
        {nav.map(({ to, label, icon: Icon }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <div
                  className={`absolute inset-0 bg-gradient-to-r from-green-500/10 to-transparent transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                />

                <div className="relative flex items-center justify-center w-8 h-8 rounded-xl shrink-0">
                  <Icon
                    className={`w-6 h-6 z-10 transition-transform duration-300 ${isActive ? 'scale-110 drop-shadow-sm' : 'group-hover:scale-110'}`}
                  />
                </div>
