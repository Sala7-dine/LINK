      ...
      <div className="card flex items-start gap-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-4 mt-2 text-sm text-gray-500">
            <span className="flex items-center gap-1"><MapPinIcon className="w-4 h-4" />{company.city || '—'}</span>
            {company.website && <a href={company.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-primary-600 hover:underline"><GlobeAltIcon className="w-4 h-4" />Site web</a>}
            <span className="flex items-center gap-1"><StarIcon className="w-4 h-4 text-yellow-400 fill-yellow-400" />{company.averageRating?.toFixed(1) || '—'} ({company.reviewCount} avis)</span>
          </div>
          ...
          {company.description && <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">{company.description}</p>}
        </div>
      </div>

      <div>
        <h2 className="text-lg font-semibold mb-4">Avis ({reviewsData?.length || 0})</h2>
        {reviewsData?.length === 0 && <div className="card text-center text-gray-400">Aucun avis pour le moment. Soyez le premier !</div>}
