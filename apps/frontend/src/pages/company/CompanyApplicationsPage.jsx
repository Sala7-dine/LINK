    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="bg-white/60 backdrop-blur-2xl p-8 rounded-[32px] border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] max-w-3xl relative overflow-hidden">
        ...
        <form
          ...
          onSubmit={handleSubmit((values) => createOfferMutation.mutate(values))}
        >
          ...
          <button
            ...
            disabled={isSubmitting || createOfferMutation.isPending}
          >
      ...
      <div className="bg-white/60 backdrop-blur-2xl p-6 sm:p-8 rounded-[32px] border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        ...
        {isLoading && (
          <p className="text-zinc-500 font-medium py-8 text-center animate-pulse">
            Chargement en cours...
          </p>
        )}
        {isError && (
          <p className="text-red-500 font-medium py-8 text-center">Erreur lors du chargement.</p>
        )}
        ...
        {!isLoading && !isError && (
          <div className="space-y-4">
            {applications.map((app) => (
              <div
                key={app._id}
                className="bg-white/50 hover:bg-white/80 transition-colors border border-white/60 shadow-sm rounded-2xl p-5 group flex flex-col gap-4"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div>
                    <p className="font-bold text-zinc-900 text-lg">
                      {app?.student?.name || 'Étudiant'}
                    </p>
                  ...
                  <select
                    ...
                    value={app.status}
                    onChange={(e) =>
                      updateStatusMutation.mutate({ id: app._id, status: e.target.value })
                    }
