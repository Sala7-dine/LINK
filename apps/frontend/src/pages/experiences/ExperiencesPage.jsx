...
export default function ExperiencesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      experienceService
        .getAll({ search: search || undefined, limit: 'all' })
        .then((r) => r.data.data.experiences),
  });
        ...
        {!isLoading && experiences.length === 0 && (
          <div className="bg-white/50 backdrop-blur-md border border-zinc-200 rounded-3xl p-12 text-center shadow-sm">
            ...
            <p className="text-zinc-500">
              Aucune expérience partagée pour le moment avec ces critères.
            </p>
          </div>
        )}
          ...
          {experiences.map((exp, i) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4 mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-zinc-900 mb-1 leading-tight">
                    {exp.companyName}
                  </h2>
              ...
              {Array.isArray(exp.technologies) && exp.technologies.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-6">
                  {exp.technologies.map((tech) => (
                    <span
                      key={tech}
                      className="text-xs px-3 py-1.5 rounded-full border border-zinc-200 text-zinc-700 font-medium"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                <div className="flex gap-4">
                  {exp.companyWebsiteUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1 text-sm font-semibold text-zinc-900 hover:text-green-600 transition-colors"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                  ...
                  {exp.companyLinkedinUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1 text-sm font-semibold text-zinc-900 hover:text-blue-600 transition-colors"
                    >
