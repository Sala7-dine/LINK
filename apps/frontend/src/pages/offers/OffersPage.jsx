        ...
        {data?.data?.offers?.map((offer, i) => (
          ...
          >
            ...
            <div className="w-16 h-16 rounded-2xl bg-zinc-50 border border-zinc-100 flex items-center justify-center flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform">
              {offer.companyLogo
                ? <img src={offer.companyLogo} alt={offer.companyName} className="w-10 h-10 object-contain" />
                : <BriefcaseIcon className="w-8 h-8 text-zinc-300" />}
            </div>
            
            {/* Content */}
            <div className="flex-1 min-w-0">
              <Link to={`/offers/${offer._id}`} className="text-xl font-bold text-zinc-900 hover:text-green-600 transition-colors block truncate mb-1">
                {offer.title}
              </Link>
              ...
              <div className="flex flex-wrap gap-2 text-xs font-semibold">
                {offer.location && (
                  <span className="flex items-center gap-1 bg-zinc-100 text-zinc-700 px-3 py-1.5 rounded-full">
                    <MapPinIcon className="w-3.5 h-3.5" />{offer.location}
                  </span>
                )}
                {offer.isRemote && <span className="bg-purple-50 text-purple-700 border border-purple-100 px-3 py-1.5 rounded-full">Remote</span>}
                <span className="capitalize bg-green-50 text-green-700 border border-green-100 px-3 py-1.5 rounded-full">{offer.contractType || 'Stage'}</span>
              </div>
              ...
              {offer.technologies?.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {offer.technologies.slice(0, 5).map((t) => (
                    <span key={t} className="text-xs font-medium border border-zinc-200 text-zinc-600 px-3 py-1 rounded-full">
                      {t}
                    </span>
                  ))}
                  {offer.technologies.length > 5 && (
                    <span className="text-xs font-bold text-zinc-400 px-2 py-1">+{offer.technologies.length - 5}</span>
                  )}
                </div>
              )}
            </div>
