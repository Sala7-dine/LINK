...
export default function UsersManagementPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      userService
        .getAll({ search: search || undefined, role: roleFilter || undefined })
        .then((r) => r.data),
  ...
  const { mutate: changeRole, isPending: isChangingRole } = useMutation({
    ...
    onError: (err) =>
      toast.error(err.response?.data?.message || 'Erreur lors du changement de role'),
  });
    ...
    <div className="space-y-8 animate-fade-in-up">
      <div>
        ...
        <p className="text-lg text-zinc-500">
          Gérez les rôles et statuts des utilisateurs de votre école.
        </p>
      </div>
      ...
      <div className="bg-white/60 backdrop-blur-2xl p-6 sm:p-8 rounded-[32px] border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-x-auto relative">
        ...
        {isLoading ? (
          <p className="text-zinc-500 py-8 text-center animate-pulse font-medium">
            Chargement en cours...
          </p>
        ) : (
            ...
            <tbody>
              {users.map((user) => {
                ...
                return (
                  <tr
                    key={user._id}
                    className="border-b border-zinc-100/50 hover:bg-white/50 transition-colors group"
                  >
                    <td className="py-4 pr-4 font-bold text-zinc-800">
                      {user.name || (
                        <span className="text-zinc-400 font-medium italic">Anonyme</span>
                      )}
                    </td>
                    ...
                    <td className="py-4 pr-4">
                      <span
                        className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider inline-block shadow-sm border
                    ...
                    <td className="py-4 text-right">
                      <button
                        className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-sm border
                         ${
                           !canEditThisUser || isTogglingActive
              ...
              {users.length === 0 && (
                <tr>
                  <td className="py-12 text-center text-zinc-500 font-medium" colSpan={5}>
